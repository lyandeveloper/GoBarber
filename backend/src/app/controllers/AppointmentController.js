...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (provider_id === req.userId) {
      return res.status(401).json({ error: 'You can not create appointments for yourself' });
    }
    ...
    if (!checkIsProvider) {
      return res.status(401).json({ error: 'You can only create appoinments with providers' });
    }
    ...
    /**
     * If everything is ok, create an appointment
     */
